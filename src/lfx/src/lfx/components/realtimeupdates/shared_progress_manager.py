...
class SharedProgressManager(Component):
    ...
    async def get_manager(self) -> Data:
        ...
        logger.info(f"Returning Data object with manager")
        return result
    ...
    async def _initialize_message(self) -> None:
        ...
        self._shared_message.content_blocks = [
            ContentBlock(title=block_title, contents=[])
        ]
    ...
    async def add_update(
        ...
        logger.info(f"Rapid update detected ({time_since_last:.0f}ms < {min_interval}ms) - debouncing for {delay*1000:.0f}ms")
    ...
    async def _send_update_now(
        ...
        # Only use actual tool output, not display text
        import json
        
        # If we have actual tool output, use it; otherwise use empty string
        # This prevents the "Calling tool..." text from accumulating
        if tool_output is not None:
            parsed_output = tool_output
            logger.info(f"[OUTPUT PARSING] tool_output type: {type(tool_output)}")
            logger.info(f"[OUTPUT PARSING] tool_output length: {len(str(tool_output))}")
            ...
            # No tool output yet (tool is starting), use empty string to avoid text accumulation
            parsed_output = ""
            logger.info(f"[OUTPUT PARSING] No tool output, using empty string")
        ...
        new_block = ContentBlock(
            title=title or ("Error" if error else "Update"),
        ...
        # Keep text empty for tool progress updates to avoid accumulation
        # But preserve text for final responses (non-tool updates)
        # Detect tool updates by checking if tool_input/output were provided
        is_tool_update = (tool_input is not None or tool_output is not None)
        if is_tool_update:
            # Tool progress update - clear text to prevent accumulation
            self._shared_message.text = ""
            logger.info("Cleared message text (tool update)")
        else:
            # Final response or non-tool update - preserve the text
            self._shared_message.text = text
            logger.info(f"Preserved message text (non-tool update): {len(text)} chars")
        
        # Ensure state remains "partial" during updates for real-time display
        self._shared_message.properties.state = "partial"
        
        # Send the update
        logger.info(f"Sending message update (skip_db_update=True)...")
        self._shared_message = await self.send_message(
            self._shared_message,
            skip_db_update=True
        )
    ...
    async def persist_to_database(self) -> Message:
        """
        Persist the current message state to the database.
        This should be called at the end of execution to save all tool calls
        and updates for display after restart.
        
        Unlike the real-time updates which use skip_db_update=True,
        this performs an actual database update to persist the message.
        """
        if not self._initialized or self._shared_message is None:
            logger.warning("Cannot persist - message not initialized")
            return self._shared_message
        
        logger.info("Persisting SharedProgressManager message to database...")
        logger.info(f"  Message ID: {self._shared_message.id}")
        logger.info(f"  Content blocks: {len(self._shared_message.content_blocks)}")
        
        # Perform actual database update (skip_db_update=False)
        self._shared_message = await self.send_message(
            self._shared_message,
            skip_db_update=False  # Actually write to database
        )
